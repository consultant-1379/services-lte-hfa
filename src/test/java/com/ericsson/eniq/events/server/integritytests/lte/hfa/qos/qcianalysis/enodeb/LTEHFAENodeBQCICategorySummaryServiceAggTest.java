...
import com.ericsson.eniq.events.server.test.queryresults.lte.handoverfailure.qos.LTEHFAQOSENodeBQCICategorySummaryResult;
import com.sun.jersey.core.util.MultivaluedMapImpl;

public class LTEHFAENodeBQCICategorySummaryServiceAggTest extends BaseDataIntegrityTest<LTEHFAQOSENodeBQCICategorySummaryResult> {
    ...
    private void insertData(final int tac, final int categoryID, final int qciID, final String time, final int instances, final String imsi)
            throws SQLException {
    ...
    private void insertAggData(final String aggTable, final long hier3ID, final int categoryID, final int qciID, final String time,
    ...
    private void insertDataForAggTestWithEventID(final int categoryID, final int qciID, final int instances, final String dateTimeID,
                                                 final String aggTable) throws SQLException {
        insertData(SAMPLE_TAC, categoryID, qciID, dateTimeID, instances, TEST_VALUE_IMSI_1);
        insertData(Integer.parseInt(TEST_VALUE_EXCLUSIVE_TAC), categoryID, qciID, dateTimeID, instances, TEST_VALUE_IMSI_1);
        insertData(SAMPLE_TAC, categoryID, qciID, dateTimeID, instances, TEST_VALUE_IMSI_2);
        insertData(Integer.parseInt(TEST_VALUE_EXCLUSIVE_TAC), categoryID, qciID, dateTimeID, instances, TEST_VALUE_IMSI_2);
        insertAggData(aggTable, TEST_VALUE_HIER3_ID, categoryID, qciID, dateTimeID, instances);
    }

    private void insertAggData(final String aggTable, final String dateTimeID) throws Exception {
        insertDataForAggTestWithEventID(CATEGORY_ID_2_PREP, TEST_VALUE_QCI_ID_1, 2, dateTimeID, aggTable);
        insertDataForAggTestWithEventID(CATEGORY_ID_2_EXEC, TEST_VALUE_QCI_ID_1, 2, dateTimeID, aggTable);

        insertDataForAggTestWithEventID(CATEGORY_ID_2_PREP, TEST_VALUE_QCI_ID_2, 2, dateTimeID, aggTable);
        insertDataForAggTestWithEventID(CATEGORY_ID_2_EXEC, TEST_VALUE_QCI_ID_2, 2, dateTimeID, aggTable);

        insertDataForAggTestWithEventID(CATEGORY_ID_2_PREP, TEST_VALUE_QCI_ID_3, 2, dateTimeID, aggTable);
        insertDataForAggTestWithEventID(CATEGORY_ID_2_EXEC, TEST_VALUE_QCI_ID_3, 2, dateTimeID, aggTable);
    }
    ...
    private void assertResult(final List<LTEHFAQOSENodeBQCICategorySummaryResult> results) {
        assertThat(results.size(), is(6));
        for (final LTEHFAQOSENodeBQCICategorySummaryResult rs : results) {
    ...
    @Test
    public void testENodeBQCICategorySummary15MinAgg() throws Exception {
        ...
        final ResultTranslator<LTEHFAQOSENodeBQCICategorySummaryResult> rt = getTranslator();
        final List<LTEHFAQOSENodeBQCICategorySummaryResult> eventResult = rt.translateResult(json, LTEHFAQOSENodeBQCICategorySummaryResult.class);
        assertResult(eventResult);
    }

    @Test
    public void testENodeBQCICategorySummaryDayAgg() throws Exception {
