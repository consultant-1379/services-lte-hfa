 ...
 *
 */
public class LTEHFAEcellEventHOStageServiceRawTest extends
        BaseDataIntegrityTest<LTEHFAEcellEventHandoverStageResultRaw> {
    ...
    private void insertData(final long hier321Id, final String eventID, final String categoryId, final int tac,
    ...
    @Test
    public void testEcellHandoverStage() throws Exception {
        final String json = getJsonResult(false);
        final ResultTranslator<LTEHFAEcellEventHandoverStageResultRaw> rt = getTranslator();
        final List<LTEHFAEcellEventHandoverStageResultRaw> eventResult = rt.translateResult(json,
                LTEHFAEcellEventHandoverStageResultRaw.class);
    ...
    //        final ResultTranslator<LTEHandoverFailureEcellEventHandoverStageResultRaw> rt = getTranslator();
    //        final List<LTEHandoverFailureEcellEventHandoverStageResultRaw> eventResult = rt.translateResult(json,
    //                LTEHandoverFailureEcellEventHandoverStageResultRaw.class);
    //        assertResult(eventResult);
    //    }

    private void assertResult(final List<LTEHFAEcellEventHandoverStageResultRaw> results) {
        assertThat(results.size(), is(2));
        for (final LTEHFAEcellEventHandoverStageResultRaw rs : results) {

            //assertEquals("Preparation", rs.getHandoverType());
            assertEquals("8", rs.getNoOfErrors());
            //assertEquals("Execution", rs.getHandoverType());
            assertEquals("8", rs.getNoOfErrors());
        }
    }
}