...
@Stateless
@Local(Service.class)
public class LTEHFAEcellEventVolumeService extends GenericService {

   private boolean isDayInterval = false;

    @Override
    public String getTemplatePath() {
        return LTE_HANDOVER_FAILURE_ECELL_EVENT_VOLUME_ANALYSIS;
    }
    ...
    public Map<String, QueryParameter> getServiceSpecificQueryParameters(final MultivaluedMap<String, String> requestParameters) {
        ...
        final long hashedIdForCell = getHashedIdForCell(requestParameters);
        if (hashedIdForCell != 0) {
            queryParameters.put(CELL_SQL_ID, QueryParameter.createLongParameter(hashedIdForCell));
        }
    ...
    public AggregationTableInfo getAggregationView(final String type) {
        return new AggregationTableInfo(HIER321_EVENTID, EventDataSourceType.AGGREGATED_15MIN, EventDataSourceType.AGGREGATED_DAY);
    }
    ...
    private long getHashedIdForCell(final MultivaluedMap<String, String> requestParameters) {
        final String hashedIdForCellAsString = requestParameters.getFirst(CELL_SQL_ID); //HIER321_ID
        ...
        if (hashedIdForCellAsString == null) {
            ...
            if (node != null) {
                ...
                hashedIdForCell = getQueryUtils().createHashIDForCell(allData[4], allData[2], allData[1], allData[0], allData[3]);
            }
        } else {
